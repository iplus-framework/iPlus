...
//                             OPC Toolbox .NET                               |
//                                                                            |
//  Filename    : ClientServerBrowser.cs                                      |
...
namespace Softing.OPCToolbox.Client{
	...
	public class ServerBrowser{
		...
		///<para lang="Visual Basic, C#, C++">The event is raised by asynchronously executing the <see cref="Browse"/> method of a <see cref="ServerBrowser"/> object.</para>
		///<para lang="Unmanaged C++">The method is called after the method <see cref="Browse"/> of a <see cref="ServerBrowser"/> object has been asynchronously executed.</para>
		///</summary>
		/// <include 
		///  file='TBNC.doc.xml' 
				...
				if(someExecutionOptions.ExecutionType == EnumExecutionType.SYNCHRONOUS){
					if(ResultCode.SUCCEEDED(res)){
						...
						for (int i = 0; i<dataCount; i++){
							...
							OTCServerData myData = (OTCServerData)Marshal.PtrToStructure((IntPtr)((int)pServerData+(structSize*i)),typeof(OTCServerData));						
	...
	public class ServerBrowserData{
		...
		internal ServerBrowserData(string clsId, string progId, string description, string progIdVersionIndependent, EnumOPCSpecification opcSpecification, string url){
			m_clsId = clsId;
			m_progId = progId;
