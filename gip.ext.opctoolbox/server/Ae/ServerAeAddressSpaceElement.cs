...
//                             OPC Toolbox .NET                               |
//                                                                            |
//  Filename    : ServerAeAddressSpaceElement.cs                              |
...
namespace Softing.OPCToolbox.Server{
	...
	public class AeAddressSpaceElement : AddressSpaceElement{
		...
		///			ctor[@name="AeAddressSpaceElement" or @name="AeAddressSpaceElement.deault"]/doc/*'
		///	/>
		...
		public override bool RemoveChild(AddressSpaceElement aChild){
			...
			if (!ResultCode.SUCCEEDED(result)){
				Application.Instance.Trace(EnumTraceLevel.WRN, EnumTraceGroup.OPCSERVER,
					"AeAddressSpaceElement.RemoveChild", "OTSRemoveAddressSpaceElement has failed with code" + result.ToString());
				return false;
			}	//	end if
			...
			if (ResultCode.SUCCEEDED(res)){
				...
				if (count > 0){
					...
					for (int i=0;i<count;i++){
						...
						children.Add(Application.Instance.AeAddressSpaceRoot.GetElementFromArray(myData.m_userData) as AeAddressSpaceElement);
					}	//	end for
				}	//	end if
	...
	public class AeAddressSpaceRoot : AddressSpaceRoot{
		...
		public AeAddressSpaceRoot(EnumAddressSpaceType anAddressSpaceType) :
			base(anAddressSpaceType,
				new AeAddressSpaceElement("AeRoot", 0, IntPtr.Zero, IntPtr.Zero)){
		}	//	end constructor
