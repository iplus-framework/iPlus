...
//                             OPC Toolbox .NET                               |
//                                                                            |
//  Filename    : ServerDaAddressSpaceElement.cs                              |
...
namespace Softing.OPCToolbox.Server {
	...
	public class DaAddressSpaceElement : AddressSpaceElement{
		...
		public override bool AddChild(AddressSpaceElement aChild){
			...
			if (child.Datatype == typeof(ulong[]) || child.Datatype == typeof(long[])){
				Application.Instance.Trace(
					EnumTraceLevel.WRN, EnumTraceGroup.OPCSERVER,
					"DaAddressSpaceElement.AddChild", "The child has invalid data type long[]");
				return false;
			}	//	end if
			
			//	only add the child if no handle assigned
			if (child.ObjectHandle == IntPtr.Zero){
		...
		public override bool RemoveChild(AddressSpaceElement aChild){
			...
			if (!ResultCode.SUCCEEDED(result)){
				Application.Instance.Trace(EnumTraceLevel.WRN, EnumTraceGroup.OPCSERVER,
					"DaAddressSpaceElement.RemoveChild", "OTSRemoveAddressSpaceElement has failed with code" + result.ToString());
				return false;
			}	//	end if
			...
			if (!ResultCode.SUCCEEDED(result)){
				Application.Instance.Trace(EnumTraceLevel.WRN, EnumTraceGroup.OPCSERVER,
					"DaAddressSpaceElement.RemoveChild", "RemoveElementFromArray has failed with code" + result.ToString());
				return false;				
			}	//	end if
		...
		public virtual int GetCacheValue(ref ValueQT aValue){
			...
			if (m_objectHandle != IntPtr.Zero && IoMode != EnumIoMode.NONE){
				...
				Win32Functions.VariantClear(valueData.m_value);
		...
		///	/>
		virtual public int QueryProperties(out ArrayList aPropertyList){
		...
		///	/>
		virtual public int ValueChanged(ValueQT aNewValue){
			
			if (this.ObjectHandle == IntPtr.Zero){
			...
			int result = OTBFunctions.OTSValuesChanged(1, new IntPtr[1]{ObjectHandle}, new OTValueData[1]{valueData});
			...
			if (ResultCode.SUCCEEDED(res)){
				...
				if (count > 0){
					...
					for (int i=0;i<count;i++){
						...
						children.Add(Application.Instance.DaAddressSpaceRoot.GetElementFromArray(myData.m_userData)  as DaAddressSpaceElement);
					}	//	end for
				}	//	end if
	...
	public class DaAddressSpaceRoot : AddressSpaceRoot{
		...
		public DaAddressSpaceRoot(EnumAddressSpaceType anAddressSpaceType) : base(
			anAddressSpaceType, 
			new DaAddressSpaceElement("DaRoot", "DaRoot", 0, IntPtr.Zero, IntPtr.Zero)){
		...
		/// <param name="anElementDetails">returned element</param>
		/// <returns>
		...
		///	/>
		public override int QueryAddressSpaceElementData(string anElementId, out AddressSpaceElement anElementDetails){
			
			anElementDetails = null;
			return (int)EnumResultCode.E_NOTIMPL;
		...
		public int ValuesChanged(DaAddressSpaceElement[] anElementList, ValueQT[] aValueList){
			...
			IntPtr[] ahObjects = new IntPtr[count];
			...
			for (int i = 0; i < count; i++){
				Win32Functions.VariantClear(aoValues[i].m_value);
		...
		internal uint ChangeItems(uint aCount, IntPtr anItemDataPointer){
			...
			for (int i = 0; i < aCount; i++){
				...
				DaAddressSpaceElement element = this.GetElementFromArray(itemData.m_object.m_userData) as DaAddressSpaceElement;
