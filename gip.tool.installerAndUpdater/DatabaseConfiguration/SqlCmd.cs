...
namespace gip.tool.installerAndUpdater
{
	...
	public class SqlCmd : IDisposable {
		...
		public SqlCmd (SqlConnection conn, int batchCount = 0) 
        {
		...
		public bool ExecuteStream(Stream stream, string basePath = null, IProgress<double> progress = null)
		{
            CurrentBatch = 1;
            _Progress = progress;
		...
		private bool ExecuteBatch (string batch, uint count) 
        {
            ...
            double percent = ((double)CurrentBatch / BatchCount) * 100;
            if (_Progress != null)
            ...
            while (count > 0) 
            {
				if (null != Executing) 
                {
					var args = new SqlCmdExecutingEventArgs (Environment, batch, percent);
					Executing (this, args);
				}
