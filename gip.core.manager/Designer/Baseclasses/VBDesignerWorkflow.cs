...
namespace gip.core.manager
{
    ...
    public abstract class VBDesignerWorkflow : VBDesigner, IACObjectDesign
    {
        ...
        protected WFLayoutCalculator WFLayoutCalculator
        {
            get
            {
        ...
        protected override void UpdateVisual()
        {
            ...
            {
                if (change.LayoutAction != VBDesigner.LayoutActionType.Delete && change.LayoutAction != VBDesigner.LayoutActionType.DeleteEdge) continue;
            ...
            foreach (var change in VisualChangeList)            //foreach (var change in VisualChangeList.Where(c => !c.IsDelete))
            {
                if (change.LayoutAction != VBDesigner.LayoutActionType.Insert) continue;
                if (changeGroup == null)
                {
                    changeGroup = this.DesignSurface.DesignContext.OpenGroup("Cut " + changedItems.Count + " elements", changedItems);
                }
        ...
        public virtual DesignItem CreateVBVisualDesignItem(VisualInfo visualInfo, IACWorkflowNode acVisualWF, DesignContext designContext, out DesignItem designItemParent)
        {
            ...
            {
                IACWorkflowNode wfNode = this.CurrentDesign as IACWorkflowNode;
                if (wfNode != null)
