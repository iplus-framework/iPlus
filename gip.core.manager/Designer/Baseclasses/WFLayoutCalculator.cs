...
namespace gip.core.manager
{
    public class WFLayoutCalculator
    {
        ...
        private void CalculateNodePos(IEnumerable<IACWorkflowEdge> parentNodes, WFItem current, WFItemList wfItemList, Size size)
        {
        ...
        private Size SizeValue(int numOfElements)
        {
        ...
        private void CalculateRootItemPosition(WFItem rootItem, Size size)
        {
            rootItem.VisualTop = size.Height - rootItem.VisualTotalHeight - 20;
            rootItem.VisualLeft = size.Width / 2 - rootItem.VisualWidth / 2;
        }

        public void WFLayoutGroup(VBDesigner.LayoutActionType layoutAction, DesignContext designContext, DesignItem designItemGroup, DesignItem designItem)
        {
            var x = designItem.ToString();
            WFItemList wfItemList = new WFItemList(designItemGroup, designItem);
            ...
            {
                DesignItem parentDesignItemGroup = designItemGroup.ParentDesignItemGroup();
                if (parentDesignItemGroup != null)
                {
                    ...
                    WFLayoutGroup(VBDesigner.LayoutActionType.Move, designContext, parentDesignItemGroup, designItemGroup);
                }
            }
