    ...
    public class PathHandlerExtension : LineExtensionBase, IKeyDown, IKeyUp
    {
            ...
            public Point TranslatedPoint
            {
                get
                {
                    if (_shape.RenderedGeometry?.Transform == null)
                        return Point;
                    // In Avalonia, transform points using the Value property
                ...
                set
                {
                    ...
                    {
                        // In Avalonia, we need to use Matrix.Invert for inverse transforms
                        var transform = _shape.RenderedGeometry.Transform;
                        var matrix = transform.Value;
                        if (matrix.TryInvert(out var inverseMatrix))
                        {
                            Point = value.Transform(inverseMatrix);
                        }
                        else
                        {
                            Point = value; // Fallback if transform can't be inverted
                        }
                    }
                }
            }
        ...
        protected class PathThumb : PointThumb
        {
            public PathThumb(int index, PathPoint pathpoint) : base()
            {
                ...
                // Use Bind method instead of SetBinding for Avalonia
...
#pragma warning restore 0414
        ...
        protected virtual Control[] BuildMenu(PathPoint pathpoint)
        {
            ...
            if (pathpoint.TargetPathPoint == null && (pathpoint.Object is LineSegment || pathpoint.Object is PolyLineSegment || pathpoint.Object is BezierSegment || pathpoint.Object is QuadraticBezierSegment || pathpoint.Object is ArcSegment))
            {
                ...
                menuItem.Click += (s, e) => ConvertPart(((Control)s).TryFindParent<PathThumb>(), PathPartConvertType.insertPoint);
                menuList.Add(menuItem);
                menuItem = new MenuItem() { Header = "to Line Segment" };
                menuItem.Click += (s, e) => ConvertPart(((Control)s).TryFindParent<PathThumb>(), PathPartConvertType.ToLineSegment);
                menuList.Add(menuItem);
                menuItem = new MenuItem() { Header = "to Bezier Segment" };
                menuItem.Click += (s, e) => ConvertPart(((Control)s).TryFindParent<PathThumb>(), PathPartConvertType.ToBezierSegment);
                menuList.Add(menuItem);
                menuItem = new MenuItem() { Header = "to Quadric Bezier Segment" };
                menuItem.Click += (s, e) => ConvertPart(((Control)s).TryFindParent<PathThumb>(), PathPartConvertType.ToQuadricBezierSegment);
                menuList.Add(menuItem);
                menuItem = new MenuItem() { Header = "to Arc Segment" };
                menuItem.Click += (s, e) => ConvertPart(((Control)s).TryFindParent<PathThumb>(), PathPartConvertType.ToArcSegment);
                menuList.Add(menuItem);
                menuList.Add(new Separator());
                menuItem = new MenuItem() { Header = "is Stroked", IsChecked = ((PathSegment)pathpoint.Object).IsStroked };
                menuItem.Click += (s, e) => ChangeIsStroked(((Control)s).TryFindParent<PathThumb>(), (MenuItem)s);
                menuList.Add(menuItem);
                // Note: IsSmoothJoin doesn't exist in Avalonia PathSegment, so we comment it out
            ...
            if (pathpoint.Object is PathFigure)
            {
                ...
                menuItem.Click += (s, e) => ChangeIsClosed(((Control)s).TryFindParent<PathThumb>(), (MenuItem)s);
                menuList.Add(menuItem);
            }
        ...
        protected virtual PathThumb CreateThumb(PlacementAlignment alignment, Cursor cursor, int index, PathPoint pathpoint, Transform transform)
        {
            ...
            // In Avalonia, we need to handle transform inversion differently
            if (transform != null)
            {
                var matrix = transform.Value;
                if (matrix.TryInvert(out var inverseMatrix))
                {
                    designerThumb.InnerRenderTransform = new MatrixTransform(inverseMatrix);
                }
            }
            ...
            // Use Avalonia event subscription instead of WeakEventManager
        ...
        private void ConvertPart(PathThumb senderThumb, PathPartConvertType convertType)
        {
            if (senderThumb.PathPoint.ParentObject is PathFigure)
            {
                ...
                // Note: PolyQuadraticBezierSegment doesn't exist in Avalonia, so we comment it out
        ...
        protected void AddGeometryPoints(List<PathPoint> list, Geometry geometry, Shape shape, Action saveDesignItem)
        {
            ...
            else if (geometry is StreamGeometry)
            {
                ...
                // In Avalonia, StreamGeometry doesn't have GetFlattenedPathGeometry, so we need to handle it differently
                // For now, we'll try to convert it to PathGeometry using a more direct approach
                PathGeometry pg = null;
                try
                {
                    // Try to create PathGeometry from string representation if possible
                    var geometryString = sg.ToString();
                    if (!string.IsNullOrEmpty(geometryString))
                    {
                        pg = PathGeometry.Parse(geometryString);
                    }
                }
                catch
                {
                    // If parsing fails, create an empty PathGeometry
                    pg = new PathGeometry();
                }
                ...
                if (d != null)
                {
                    ...
                    saveDesignItem = () =>
                    {
                        // In Avalonia, we don't have GeometryConverter, so we use the ToString() method
                        dClosure.ParentProperty.SetValue(PathGeometry.Parse(pg.ToString()));
                    };
                }
                ...
                if (geometry != null)
                {
                    foreach (var figure in g.Figures)
                    {
                        ...
                        foreach (var s in figure.Segments)
                        {
                            ...
                            // Note: PolyQuadraticBezierSegment doesn't exist in Avalonia, commenting out
        ...
        public void KeyDownAction(object sender, KeyEventArgs e)
        {
            if (_selectedPoints.Count > 0)
            {
                ...
                var dx1 = (e.Key == Key.Left) ? IsKeyDown(Key.LeftShift) ? -10 : -1 : 0;
                var dy1 = (e.Key == Key.Up) ? IsKeyDown(Key.LeftShift) ? -10 : -1 : 0;
                var dx2 = (e.Key == Key.Right) ? IsKeyDown(Key.LeftShift) ? +10 : +1 : 0;
                var dy2 = (e.Key == Key.Down) ? IsKeyDown(Key.LeftShift) ? +10 : +1 : 0;
