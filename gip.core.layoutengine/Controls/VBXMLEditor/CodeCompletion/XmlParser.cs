    ...
    public static class XmlParser
    {
        ...
        public static XmlElementPath GetActiveElementStartPath(string xml, int index)
        {
            ...
            if (elementText != null)
            {
                ...
                NamespaceURI elementNamespace = GetElementNamespace(xml, elementName.Prefix);
        ...
        /// <param name="xml">This string must start at the 
        /// element we are interested in.</param>
        static NamespaceURI GetElementNamespace(string xml, string prefix = "")
        {
            NamespaceURI namespaceURI = new NamespaceURI();

            if (!string.IsNullOrEmpty(prefix))
            {
                string prefixName = "xmlns:" + prefix + "=\"";

                if (xml.Contains(prefixName))
                {
                    int sIndx = xml.IndexOf(prefixName);
                    string part = xml.Substring(sIndx + prefixName.Length);
                    int lIndx = part.IndexOf("\"");
                    string ns = part.Substring(0, lIndx);
                    namespaceURI.Namespace = ns;
