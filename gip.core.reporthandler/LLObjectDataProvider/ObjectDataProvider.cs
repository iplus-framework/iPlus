...
{
    internal interface ISupplyParentTable
    {
        ISupplyParentTable ParentTable
        {
            get;
        }
    ...
    public sealed class LLHandleEnumerablePropertyEventArgs : EventArgs
    {
        ...
        public LLHandleEnumerablePropertyEventArgs(string propertyPath, bool cancelRecursion, Type objectType)
        {
            this.PropertyPath = propertyPath;
            this.CancelRecursion = cancelRecursion;
	...
	public sealed class ObjectDataProvider : IDataProvider, ICanHandleUsedIdentifiers
    {
        ...
        private ACQueryDefinition _ACQueryDef;
        ...
        public static readonly ACMonitorObject _90000_LockPropertyTypeCache = new ACMonitorObject(90000);
        private static Dictionary<Type, PropertyType> _propertyTypeCache = new Dictionary<Type, PropertyType>();
		private bool _initialized;
        public event EventHandler<LLHandleEnumerablePropertyEventArgs> HandleEnumerableProperty;
        ...
        #region c'tors
        public ObjectDataProvider(object source, ACQueryDefinition queryDefinition, int maximumRecursionDepth)
        {
            this.MaximumRecursionDepth = maximumRecursionDepth;
            this._source = source;
            this._ACQueryDef = queryDefinition;
        ...
        /// <summary>
        /// Provider goes along the ACQueryDefinition-Structure and adds Relations which have max. 1 distance from ACQueryDefinition-Object
        /// </summary>
        /// <param name="source"></param>
        /// <param name="queryDefinition">maximumRecursionDepth is 1 when ACQueryDefinition is passed</param>
        public ObjectDataProvider(object source, ACQueryDefinition queryDefinition)
            : this(source, queryDefinition, 1)
        {
        }

        public ObjectDataProvider(object source, int maximumRecursionDepth) 
            : this(source, null, maximumRecursionDepth)
		{
		}
        ...
        internal static PropertyType GetPropertyTypeFromType(Type type)
		{
            ...
            using (ACMonitor.Lock(_90000_LockPropertyTypeCache))
            {
		...
		private void Init()
		{
			...
			this.BuildDom(enumerable2, enumerable, null, sourceType, this.RootTableName, null, "1", null, 0, this.RootTableName, this._ACQueryDef);
		}
        ...
        private void OnHandleEnumerableProperty(LLHandleEnumerablePropertyEventArgs e)
		{
		...
		private void BuildDom(IEnumerable sourceForStructureParsing, IEnumerable source, IEnumerable parentSource, Type sourceType, string tableName, string propertyName, string navigationId, ISupplyParentTable parentTable, int recursionDepth, string propertyPath, ACQueryDefinition queryDef)
		{
			...
			this._initialized = true;
            if (queryDef == null && recursionDepth > this.MaximumRecursionDepth)
			{
            ...
            if (queryDef != null)
                acClass = queryDef.TypeACClass;
            else if (t != null)
                acClass = Database.GlobalDatabase.GetACType(t);
            ...
            LLHandleEnumerablePropertyEventArgs handleEnumerablePropertyEventArgs = new LLHandleEnumerablePropertyEventArgs(propertyPath, false, t);
				...
				try
				{
					...
					if (enumerator2.MoveNext())
					{
						...
						catch (ArgumentOutOfRangeException ec)
						{
                            ...
                            this.Root().Messages.LogException("ObjectDataProvider", "BuildDom", msg);
                        }
					}
			...
			foreach (PropertyDescriptor propertyDescriptor in propertyDescriptorCollection)
			{
					...
					if (enumerable2 != null)
					{
						...
						try
						{
							...
							catch (ArgumentOutOfRangeException ec)
							{
                                    ...
                                    this.Root().Messages.LogException("ObjectDataProvider", "BuildDom(10)", msg);
						...
						if (((IDataProvider)this).GetRelation(relationName) == null)
						{
                            ...
                            ACQueryDefinition childQueryDef = null;
                            int nextRecursionDepth = recursionDepth + 1;
                            if (queryDef != null)
                            {
                                childQueryDef = queryDef.ACQueryDefinitionChilds.Where(c => c.ChildACUrl == uniqueTableName).FirstOrDefault();
                                if (childQueryDef != null)
                                    nextRecursionDepth = 0;
                            }
                            this.BuildDom(null, enumerable2, source, genericEnumerableType, uniqueTableName, propertyDescriptor.Name, navigationId + ".1", obj as ISupplyParentTable, nextRecursionDepth, propertyPath + ">" + uniqueTableName, childQueryDef);
						}
					}
					break;
				}
				case PropertyType.GenericEnumerable:
				{
					...
					if (((IDataProvider)this).GetRelation(relationName2) == null)
					{
                        ...
                        if (queryDef != null)
                        {
                            childQueryDef = queryDef.ACQueryDefinitionChilds.Where(c => c.ChildACUrl == uniqueTableName2).FirstOrDefault();
                            if (childQueryDef != null)
                                nextRecursionDepth = 0;
                        }
                        this.BuildDom(null, propertyDescriptor.GetValue(obj2) as IEnumerable, source, propertyType, uniqueTableName2, propertyDescriptor.Name, navigationId + ".1", obj as ISupplyParentTable, nextRecursionDepth, propertyPath + ">" + uniqueTableName2, childQueryDef);
					}
					break;
				}
				case PropertyType.ListSource:
					...
					if (listSource != null)
					{
						...
						case PropertyType.Enumerable:
						{
							...
							if (list.Count != 0)
							{
								...
								if (((IDataProvider)this).GetRelation(relationName3) == null)
								{
                                    ...
                                    if (queryDef != null)
                                    {
                                        childQueryDef = queryDef.ACQueryDefinitionChilds.Where(c => c.ChildACUrl == uniqueTableName3).FirstOrDefault();
                                        if (childQueryDef != null)
                                            nextRecursionDepth = 0;
                                    }
                                    this.BuildDom(null, list, source, type3, uniqueTableName3, propertyDescriptor.Name, navigationId + ".1", obj as ISupplyParentTable, nextRecursionDepth, propertyPath + ">" + uniqueTableName3, childQueryDef);
								}
							}
							break;
						}
						case PropertyType.GenericEnumerable:
						{
							...
							if (((IDataProvider)this).GetRelation(relationName4) == null)
							{
                                ...
                                if (queryDef != null)
                                {
                                    childQueryDef = queryDef.ACQueryDefinitionChilds.Where(c => c.ChildACUrl == uniqueTableName4).FirstOrDefault();
                                    if (childQueryDef != null)
                                        nextRecursionDepth = 0;
                                }
                                this.BuildDom(null, list, source, type3, uniqueTableName4, propertyDescriptor.Name, navigationId + ".1", obj as ISupplyParentTable, nextRecursionDepth, propertyPath + ">" + uniqueTableName4, childQueryDef);
							}
							break;
						}
						default:
				...
				case PropertyType.Class:
				{
					...
					if (((IDataProvider)this).GetRelation(relationName5) == null)
					{
                        ...
                        if (queryDef != null)
                        {
                            childQueryDef = queryDef.ACQueryDefinitionChilds.Where(c => c.ChildACUrl == uniqueTableName5).FirstOrDefault();
                            if (childQueryDef != null)
                                nextRecursionDepth = 0;
                        }
                        this.BuildDom(null, list2, source, type5, uniqueTableName5, propertyDescriptor.Name, navigationId + ".1", obj as ISupplyParentTable, nextRecursionDepth, propertyPath + ">" + uniqueTableName5, childQueryDef);
					}
					break;
				}
				}
