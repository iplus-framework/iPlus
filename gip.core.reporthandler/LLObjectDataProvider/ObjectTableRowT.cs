	...
	internal class ObjectTableRow<T> : ITableRow
	{
		...
		public ObjectTableRow(object source, ITypedList typedList, ObjectTable<T> table, string navigationId)
		{
			...
			catch (InvalidCastException innerException)
			{
                ...
                if (innerException.InnerException != null && innerException.InnerException.Message != null)
                    msg += " Inner:" + innerException.InnerException.Message;

                this.Root().Messages.LogException("ObjectTableRow<T>", "ObjectTableRow", msg);
		...
		private void InitColumns()
		{
						...
						if (this._table.UsedIdentifiers == null || this._table.UsedIdentifiers.Contains(propertyDescriptor.DisplayName))
						{
							...
							if (ObjectDataProvider.GetPropertyTypeFromType(propertyType) == PropertyType.Column)
							{
								...
								catch (TargetInvocationException e)
								{
                                    ...
                                    this.Root().Messages.LogException("ObjectTableRow<T>", "InitColumns", msg);
                                }
							}
						}
					}

                    Type hasEntityInterface = typeof(T).GetInterface(typeof(IACEntityProperty).Name);
                    if (hasEntityInterface != null && this._table.ACClass != null)
                    {
                        foreach (ACClassProperty acPropertyExt in this._table.ACClass.ExtendedProperties)
                        {
                            try
                            {
                                if (this._source != null)
                                    this._columns.Add(new ObjectTableColumn(acPropertyExt.ACIdentifier, acPropertyExt.ObjectFullType, (this._source as IACEntityProperty)[acPropertyExt.ACIdentifier]));
                                else
                                    this._columns.Add(new ObjectTableColumn(acPropertyExt.ACIdentifier, acPropertyExt.ObjectFullType, DBNull.Value));
                            }
                            catch (TargetInvocationException e)
                            {
                                this._columns.Add(new ObjectTableColumn(acPropertyExt.ACIdentifier, acPropertyExt.ObjectFullType, DBNull.Value));
                                ...
                                this.Root().Messages.LogException("ObjectTableRow<T>", "InitColumns(10)", msg);
                            }
                        }
                    }
			...
			if (this._table.UsedIdentifiers == null || this._table.UsedIdentifiers.Contains(Const.Value))
			{
				this._columns.Add(new ObjectTableColumn(Const.Value, typeof(T), this._source));
			}
			IL_20F:
