...
{
    public class ModbusItem : IACObject, INotifyPropertyChanged
    {
        #region c'tors
        public ModbusItem(IACPropertyNetServer acProperty, string itemAddr, ModbusSubscr parentSubscription)
        {
            _ItemAddr = itemAddr;
            _ACProperty = acProperty;
            //_ParentSubscription = new ACRef<ModbusSubscr>(parentSubscription, this);
            _ParentSubscription = parentSubscription;
            _ACProperty.ValueUpdatedOnReceival += OnSendValueToPLC;
            _IsValidItemSyntax = ItemSyntaxResolver.Resolve(_ItemAddr, out _ItemTableType, out _ItemVarType, out _ItemDBNo, out _ItemStartByteAddr, out _ItemLength, out _ItemBitNo);
        }
        #endregion

        #region Properties
        //protected ACRef<ModbusSubscr> _ParentSubscription = null; ACRef nicht notwendig, da bei Restart sowieso Items neu geladen werden müssen
        protected ModbusSubscr _ParentSubscription = null;
        ...
        private TableType _ItemTableType;
        public TableType ItemTableType
        {
            get
            {
                return _ItemTableType;
            }
        }
        ...
        public void Refresh(ModbusDataBlock dataBlock, ref byte[] readResult, int atIndex, int dbReadStartIndex)
        {
            ...
            switch (ItemVarType)
            {
                ...
                case VarType.Byte:
                    ...
                    break;
                case VarType.Word:
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.Word.FromByteArray(readExtract, ParentSubscription.Endianess), isEqualInRAM);
                    break;
                case VarType.Int:
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.Int.FromByteArray(readExtract, ParentSubscription.Endianess), isEqualInRAM);
                    break;
                case VarType.DWord:
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.DWord.FromByteArray(readExtract, ParentSubscription.Endianess), isEqualInRAM);
                    break;
                case VarType.DInt:
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.DInt.FromByteArray(readExtract, ParentSubscription.Endianess), isEqualInRAM);
                    break;
                case VarType.Real:
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.Real.FromByteArray(readExtract, ParentSubscription.Endianess), isEqualInRAM);
                    break;
                case VarType.String:
                    _StringLen = readExtract[0];
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.String.FromByteArray(readExtract), isEqualInRAM);
                    break;
                case VarType.Base64String:
                    _StringLen = readExtract[0];
                    OnSetValueFromPLC(gip.core.communication.modbus.Types.String.FromByteArray(readExtract, true), isEqualInRAM);
                    break;
            }
        }
                ...
                if (_ReadLockCounter <= 0)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (datamodel.Database.Root != null && datamodel.Database.Root.Messages != null && datamodel.Database.Root.InitState == ACInitState.Initialized)
                            datamodel.Database.Root.Messages.LogException("ModbusItem", "OnSetValueFromPLC", msg);
                    }
                    finally
                    {
                        //_ReSendLock = false;
        ...
        void OnSendValueToPLC(object sender, ACPropertyChangedEventArgs e, ACPropertyChangedPhase phase)
        {
            ...
            using (ACMonitor.Lock(_30150_ReadItemLock))
            {
                _ReadLockCounter = 2;
                _WriteUpdates++;
                (ParentSubscription.ParentACComponent as ModbusSession).SendItem(this);
            }
        }
