...
namespace gip.ext.chart.Common
{
	...
	public struct DataRect : IEquatable<DataRect>
	{
		...
		public DataRect(double x, double y, double width, double height)
		{
			if ((width < 0) || (height < 0))
			{
                throw new ArgumentException(gip.ext.chart.Properties.Resources.WidthAndHeightCannotBeNegative);
			}
			this.xMin = x;
			this.yMin = y;
			this.width = width;
		...
		public Point Location
		{
			...
			set
			{
				if (IsEmpty)
				{
                    throw new InvalidOperationException(gip.ext.chart.Properties.Resources.CannotModifyEmptyRect);
				}
				xMin = value.X;
				yMin = value.Y;
			}
