	...
	public class LineGraph : PointsGraphBase
	{
		...
		public Pen LinePen
		{
            ...
            {   SetValue(LinePenProperty, value);
                _StoredStrokeThickness = value.Thickness;
            }
		}
        ...
        private double _StoredStrokeThickness = 0;
        /// <summary>
        /// Gip-Extension: IsLineGraphVisible
        /// </summary>
        private bool _IsLineGraphVisible = true;
        public bool IsLineGraphVisible
        {
            get 
            { 
                return _IsLineGraphVisible; 
            }
            set
            {
                bool changed = false;
                if (_IsLineGraphVisible != value)
                    changed = true;
                _IsLineGraphVisible = value;
                if (changed)
                {
                    if (_IsLineGraphVisible)
                        StrokeThickness = _StoredStrokeThickness;
                    else
                        StrokeThickness = 0;
                    Update();
                }
