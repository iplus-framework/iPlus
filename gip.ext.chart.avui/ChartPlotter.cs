...
namespace gip.ext.chart.avui
{
    ...
    public class ChartPlotter : Plotter2D
	{
		...
		public ChartPlotter()
		{
            axisGrid = new AxisGrid(AxisGridBrush);
		...
		private static void VerifyAxisType(AxisPlacement axisPlacement, AxisType axisType)
		{
			...
			if (!result)
                throw new ArgumentException(gip.ext.chart.avui.Properties.Resources.InvalidAxisPlacement);
		}
        ...
        /// <param name="e"></param>
        public override void OnMouseDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            if (e.Source != null)
            {
                if (e.Source is LegendItem)
                {
                    LegendItem item = (e.Source as LegendItem);
                    item.Description.IsLineGraphVisible = !item.Description.IsLineGraphVisible;
                }
            }
        }
        ...
        {
            get { return (Brush)GetValue(AxisGridBrushProperty); }
            set 
            { 
                SetValue(AxisGridBrushProperty, value);
                if (AxisGrid != null)
                    AxisGrid.AxisGridBrush = value;
            }
        }
