	...
	public static class ScreenshotHelper
	{
		...
		internal static BitmapEncoder GetEncoderByExtension(string extension)
		{
			switch (extension)
			{
				...
				default:
                    throw new ArgumentException(gip.ext.chart.avui.Properties.Resources.CannotDetermineImageTypeByExtension, "extension");
			}
		}
        ...
        public static BitmapSource CreatePrintableBitmap(UIElement uiElement, Int32Rect screenshotSource, int dpi = 96)
        {
            Window window = Window.GetWindow(uiElement);
            if (window == null)
            {
		...
		internal static void SaveBitmapToStream(BitmapSource bitmap, Stream stream, string fileExtension)
		{
			...
			if (String.IsNullOrEmpty(fileExtension))
                throw new ArgumentException(gip.ext.chart.avui.Properties.Resources.ExtensionCannotBeNullOrEmpty, fileExtension);
		...
		internal static void SaveBitmapToFile(BitmapSource bitmap, string filePath)
		{
			if (String.IsNullOrEmpty(filePath))
                throw new ArgumentException(gip.ext.chart.avui.Properties.Resources.FilePathCannotbeNullOrEmpty, "filePath");
