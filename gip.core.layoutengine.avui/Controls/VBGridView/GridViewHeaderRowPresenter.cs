...
namespace gip.core.layoutengine.avui
{
    ...
    public class GridViewHeaderRowPresenter : GridViewRowPresenterBase
    {
        ...
        public static readonly StyledProperty<ControlTheme> ColumnHeaderContainerStyleProperty =
                GridView.ColumnHeaderContainerStyleProperty.AddOwner<GridViewHeaderRowPresenter>();
        ...
        public static readonly StyledProperty<IDataTemplate> ColumnHeaderTemplateSelectorProperty =
            GridView.ColumnHeaderTemplateSelectorProperty.AddOwner<GridViewHeaderRowPresenter>();
        ...
        /// <summary>
        /// AllowsColumnReorder StyledProperty
        /// </summary>
        ...
        /// <summary>
        /// ColumnHeaderContextMenu StyledProperty
        /// </summary>
        ...
        /// <summary>
        /// ColumnHeaderToolTip StyledProperty
        /// </summary>
        ...
        private static void LocalPropertyChanged(GridViewHeaderRowPresenter sender, AvaloniaPropertyChangedEventArgs e)
        {
            ...
            //    Helper.CheckTemplateAndTemplateSelector("GridViewHeaderRowPresenter", ColumnHeaderTemplateProperty, ColumnHeaderTemplateSelectorProperty, sender);
            //}
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            if (columns != null)
            {
                ...
                {
                    Control child = children[GetVisualIndex(i)];
        ...
        /// on each child.
        /// </summary>
        /// <param name="finalSize">Size the GridViewHeaderRowPresenter will assume.</param>
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (columns != null)
            {
                ...
                for (int i = 0; i < columns.Count; ++i)
                {
                    ...
                    rect = new Rect(accumulatedWidth, 0.0, childArrangeWidth, finalSize.Height);
            ...
            if (_isHeaderDragging)
            {
                ...
                _indicator.Arrange(new Rect(pos, new Size(_indicator.DesiredSize.Width, finalSize.Height)));
            }
            ...
            if (e.GetCurrentPoint(this).Properties.IsLeftButtonPressed)
            {
                ...
                if (_prepareDragging)
                {
                    ...
                    else // NOTE: Not-Dragging/Dragging should be divided into two stages in PointerMoved
                    {
                        ...
                        bool isDisplayingFloatingHeader = IsPointerPositionValid(_floatingHeader, _currentPos, 2.0);

                        // floating header and indicator are visible/invisible at the same time
        ...
        protected override void OnPointerCaptureLost(PointerCaptureLostEventArgs e)
        {
            ...
            // OnPointerCaptureLost is invoked before OnPointerReleased, so we need to distinguish
            // the cause of capture lose
            //      if LeftButton is pressed when lost pointer capture, we treat it as cancel
            //      Because GridViewHeaderRowPresenter never capture Pointer (GridViewColumnHeader did this),
            //      the captured pointer is always null
            if (e.Pointer?.Captured != null && _isHeaderDragging)
            {
        ...
        internal void MakeParentItemsControlGotFocus()
        {
            if (_itemsControl != null && !_itemsControl.IsKeyboardFocusWithin)
            {
                ...
                {
                    // In Avalonia, we'd need to find the container and focus it differently
                    var container = parent.ContainerFromItem(parent.SelectedItem);
        ...
        /// <summary>
        /// Creates AutomationPeer (<see cref="Control.OnCreateAutomationPeer"/>)
        /// </summary>
        ...
        private GridViewColumnHeader CreateAndInsertHeader(GridViewColumn column, int index)
        {
            ...
            //
            // NOTE: when theme changed, all properties, templates and styles will be reevaluated.
            ...
            //    </GridViewColumn>
            //
            // In both cases, we need to disconnect them from the visual and logical tree before
            // they got inserted into the new tree.
            //
            if (header != null)
            {
                StyledElement d = header as StyledElement;
                ...
                if (d != null)
                {
                    ...
                    {
                        Visual parent = headerAsVisual.GetVisualParent();
                    ...
                    ILogical logicalParent = d.GetLogicalParent();

                    if (logicalParent != null)
                    {
                        // In Avalonia not possible. But should be already done some lines above
                        //((ILogical)logicalParent).LogicalChildren.Remove(d);
                    }
                }
            }
            ...
            // Hookup _itemsControl.KeyDown event for canceling resizing if user presses 'Esc'.
            HookupItemsControlKeyboardEvent(headerContainer);

            InternalChildren.Insert(index, headerContainer);

            // NOTE: the order here is important!
            // Need to add headerContainer into visual tree first, then apply Theme, Template etc.
        ...
        private void RenewEvents()
        {
            ...
            _headerSV = this.FindAncestorOfType<ScrollViewer>();
            ...
            if (oldIC != _itemsControl)
            {
                ...
                if (_itemsControl != null)
                {
                    ...
                    // NOTE: headers will hookup the KeyDown event later in CreateAndInsertHeader.
                }
            }
        ...
        private void OnMasterScrollChanged(object sender, ScrollChangedEventArgs e)
        {
            ...
            {
                _headerSV.Offset = new Vector(e.OffsetDelta.X, _headerSV.Offset.Y);
            }
        }
        ...
        private void OnHeaderScrollChanged(object sender, ScrollChangedEventArgs e)
        {
            ...
            {
                _mainSV.Offset = new Vector(e.OffsetDelta.X, _mainSV.Offset.Y);
            }
        }
        ...
        private void AddPaddingColumnHeader()
        {
            ...
            // In Avalonia, Focusable is always available
        ...
        private void AddIndicator()
        {
            ...
            // In Avalonia, we use FuncControlTemplate instead of FrameworkElementFactory
            var template = new FuncControlTemplate<Separator>((control, scope) =>
            {
                var border = new Border
                {
                    Background = new SolidColorBrush(Color.FromUInt32(0xFF000080))
                };
                return border;
            });
        ...
        private void AddFloatingHeader(GridViewColumnHeader srcHeader)
        {
            ...
            {
                throw new ArgumentException($"ListView missing parameterless constructor for type {headerType}", e);
            }
        ...
        private void UpdateFloatingHeader(GridViewColumnHeader srcHeader)
        {
            ...
            _floatingHeader.Theme = srcHeader.Theme;
        ...
        /// <summary>
        /// This method is invoked when pointer move and left button is pressed.
        /// This method judges if the pointer move exceeds a threshold to start re-order.
        /// </summary>
        /// <param name="currentPos">The current pointer position, relative to GridViewHeaderRowPresenter</param>
        ...
        private GridViewColumnHeader FindHeaderByColumn(GridViewColumn column)
        {
            ...
            if (columns != null && children.Count > columns.Count)
            {
                ...
                if (index != -1)
                {
                    // Because column headers is generated from right to left
                    ...
                    {
                        // NOTE: if user change the GridViewColumn.Header/HeaderTheme
        ...
        // If parameter 'findNearestColumn' is true, find the nearest column relative to pointer position
        ...
        // Update header Content, Theme, ContentTemplate, ContextMenu and ToolTip
        ...
        // Update Theme, ContextMenu and ToolTip properties for padding header.
            ...
            )
        {
                ...
                && header.Role == GridViewColumnHeaderRole.Padding)
            {
                ...
                ControlTheme theme = ColumnHeaderContainerStyle;
                if (theme != null && !theme.TargetType.IsAssignableFrom(typeof(GridViewColumnHeader)))
                {
                    // use default theme for padding header in this case
                    header.Theme = null;
                    return;
                }
            }
        ...
        private void FinishHeaderDrag(bool isCancel)
        {
            ...
            if (!isCancel)
            {
                ...
                // Revise the destination column index
        ...
        // current pointer position (position relative to GridViewHeaderRowPresenter)
        ...
        // indicating a pointer down, ready to drag the header
        ...
        {
            // Properties on GridViewHeaderRowPresenter
            ...
            // Properties on GridViewColumnHeader
            new AvaloniaProperty[] {
                GridViewColumnHeader.ThemeProperty,
                GridViewColumnHeader.ContentTemplateProperty,
                null, // ContentTemplateSelector not used in Avalonia the same way
                null, // ContentStringFormat handled differently in Avalonia
        ...
        static GridViewHeaderRowPresenter()
        {
            ...
            ColumnHeaderContainerStyleProperty.Changed.AddClassHandler<GridViewHeaderRowPresenter>(LocalPropertyChanged);
            ColumnHeaderTemplateProperty.Changed.AddClassHandler<GridViewHeaderRowPresenter>(LocalPropertyChanged);
            ColumnHeaderTemplateSelectorProperty.Changed.AddClassHandler<GridViewHeaderRowPresenter>(LocalPropertyChanged);
            ColumnHeaderStringFormatProperty.Changed.AddClassHandler<GridViewHeaderRowPresenter>(LocalPropertyChanged);
            ColumnHeaderContextMenuProperty.Changed.AddClassHandler<GridViewHeaderRowPresenter>(LocalPropertyChanged);
            ColumnHeaderToolTipProperty.Changed.AddClassHandler<GridViewHeaderRowPresenter>(LocalPropertyChanged);
        }
    }
}
