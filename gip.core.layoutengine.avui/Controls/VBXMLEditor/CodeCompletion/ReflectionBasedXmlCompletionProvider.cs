...
namespace gip.core.layoutengine.avui.CodeCompletion
{
    ...
    public class ReflectionBasedXmlCompletionProvider : ICompletionDataProvider
    {
        ...
        {
            _typeFinder = XamlTypeFinder.CreateAvaloniaTypeFinder();
        ...
        {
            // The XamlTypeFinder already registers core Avalonia assemblies
                ...
                try
                {
                    ...
                    foreach (var type in assemblyTypes)
                    {
                        if (type.IsPublic && !type.IsAbstract)
                        {
                            ...
                            string typeXmlNamespace = _typeFinder.GetXmlNamespaceFor(assembly, type.Namespace ?? string.Empty);
        ...
        private ICompletionData[] GetAttributeCompletionData(string text)
        {
            ...
            // Get Avalonia dependency properties
            var dependencyProperties = GetAvaloniaProperties(elementType);
            foreach (var dp in dependencyProperties)
            {
                if (addedAttributes.Add(dp.Name))
                {
                    completionData.Add(new XmlCompletionData(dp.Name, $"Avalonia Property: {dp.PropertyType.Name}", XmlCompletionData.DataType.Property));
                }
            }
        ...
        private IEnumerable<AvaloniaProperty> GetAvaloniaProperties(Type type)
        {
            var properties = new List<AvaloniaProperty>();
            ...
            try
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    {
                        var dp = field.GetValue(null) as AvaloniaProperty;
                        if (dp != null)
                        {
                            properties.Add(dp);
                        }
        ...
        private IEnumerable<AvaloniaProperty> GetAttachedProperties(Type type)
        {
            var properties = new List<AvaloniaProperty>();
        ...
        private Type GetPropertyType(Type elementType, string propertyName)
        {
            ...
            if (dpField != null && typeof(AvaloniaProperty).IsAssignableFrom(dpField.FieldType))
            {
                var dp = dpField.GetValue(null) as AvaloniaProperty;
                return dp?.PropertyType;
            }
