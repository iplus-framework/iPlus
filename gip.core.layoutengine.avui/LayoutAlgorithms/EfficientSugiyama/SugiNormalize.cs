...
namespace gip.core.layoutengine.avui.EfficientSugiyama
{
    public partial class SugiLayoutAlgorithm<TVertex, TEdge>
    {
        ...
        private void DoWidthAndHeightOptimization()
        {
            ...
            if (_actualWidthPerHeight <= _WidthPerHeight)
                return;
        ...
        {
            double desiredWidth = _actualHeight * _WidthPerHeight;
            ...
            else
            {
                ...
                if (insertedLayerIndex > 0)
                {
                    ...
                    {
                        width += Math.Max(0, _Graph.OutDegree(vertex) - 1) * _LayerDistance;
                    }
                    c += 1;
                }
                ...
                _actualHeight += height + _LayerDistance;
        ...
        private void CreateLayerWHOptInfos()
        {
            ...
            foreach (var layer in _layers)
            {
                ...
                layerInfo.LayerWidth += Math.Max(0, layer.Count - 1) * _VertexDistance;
                ...
                _actualHeight += layerInfo.LayerHeight + _LayerDistance;
            ...
            _actualHeight -= _LayerDistance;
            _actualWidth -= _VertexDistance;
        }
        ...
        private void CreateVertexWHOptInfos()
        {
            ...
            foreach (var vertex in _Graph.Vertices)
            {
                ...
                whOptInfo.Value = vertex.Size.Width - Math.Max(0, _Graph.InDegree(vertex) - 1) * _VertexDistance;
                whOptInfo.Cost = vertex.Size.Width - Math.Max(0, _Graph.OutDegree(vertex) - 1) * _VertexDistance;
        ...
        private void CreateDummyVerticesAndSegments()
        {
            foreach (var edge in _Graph.Edges.ToList())
            {
                ...
                _Graph.RemoveEdge(edge);
                bool notReversed = edge.Source.OriginalVertex as datamodel.IACObject == edge.OriginalEdge.SourceParent && 
                                   edge.Target.OriginalVertex as datamodel.IACObject == edge.OriginalEdge.TargetParent;
