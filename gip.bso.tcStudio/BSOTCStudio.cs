...
namespace gip.bso.tcStudio
{
    ...
    public class BSOTCStudio : ACBSO
    {
        ...
        string[] _DirSeperatorArr = new string[] { GCL.Delimiter_DirSeperator };
        ...
        string _acClassInfoMovedName = String.Empty;

        private const string C_RegionStartName = "//VBRegionStart";
        public const string C_RegionWarningComment = "//Variobatch-autogenerated (WARNING: Do not write any code in Variobatch autogenerated region, because that code may be lost on next synchronization!!!!)";
        public const string C_VBRegionStart = C_RegionWarningComment + "\r\n" + C_RegionStartName;

        private string VBRegionStart(string spaces = "")
        {
            string startRegion = spaces + C_RegionWarningComment + System.Environment.NewLine + spaces + C_RegionStartName;
            return startRegion;
        }

        public const string C_VBRegionEnd = "//VBRegionEnd";
        public const string C_4spaces = "    ";
        public const string C_8spaces = "        ";
        public const string C_ST_Code_RefMain = " REF= " + GCL.cMainIdentifier + GCL.cADSPathSeparator;
        public const string C_ST_Code_RefMainChild = C_ST_Code_RefMain + GCL.cADSInstancePrefix;

        const string _ReferenceTo = " REFERENCE TO ";
        public const string C_ST_Code_IF_Autocomp_THEN = GCL.cST_Keyword_IF + " Automation_components_gip.GVL.gInstanceIDCount = 0 " + GCL.cST_Keyword_THEN;
        public const string C_ST_Code_IF_NOT_InitState_THEN = GCL.cST_Keyword_IF + " NOT(ACInitState = ACInitStateEnum.Unchanged) " + GCL.cST_Keyword_THEN;
        ...
        public ACValueItemList VisualStudioList
        {
            get
            {
                ...
                {
                    _VisualStudioList = new ACValueItemList(String.Empty);
                ...
                if (pou != null)
                {
                    foreach (ITcSmTreeItem tcMethod in ITcSmTreeItemList(pou))
                    {
                        ...
                        if (ITCAction(() => tcMethod.ItemType) == (int)ItemType.Method 
                                                                    ...
                                                                    && itemName != GCL.cMethodName_OnMainEnter
                                                                    && itemName != GCL.cMethodName_OnMainExit
                                                                    && itemName != GCL.cMethodName_FB_childinit
        ...
        private void CompareProjectRecursive(ACClassInfoWithItems currentInfo, ITcSmTreeItem currentTreeItem, string parentName)
        {
            ...
            foreach (ACClassInfoWithItems infoItem in currentInfo.Items)
            {
                ...
                string name = parentName + GCL.Delimiter_DirSeperator + infoItem.ACIdentifier;
                if (infoItem.Items.Any())
                {
                    ...
                    if (folder != null)
                        treeItem = FindTreeItem(parentName + GCL.Delimiter_DirSeperator + infoItem.ACIdentifier, folder);
                    else
                        treeItem = FindTreeItem(parentName + GCL.Delimiter_DirSeperator + infoItem.ACIdentifier, currentTreeItem);
        ...
        private void CompareTCProject(ITcSmTreeItem treeItem, ACClassInfoWithItems currentInfo)
        {
            ...
            if (ITCSmTreeItemChildCount(treeItem) - 1 > currentInfo.Items.OfType<ACClassInfoWithItems>().Count(c => (Global.ChangeTypeEnum)c.IconState != Global.ChangeTypeEnum.Added))
            {
                foreach (ITcSmTreeItem tItem in ITcSmTreeItemList(treeItem))
                {
                    ...
                    string tItemName = ITCAction(() => tItem.Name).Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault();
        ...
        private void AddTCProjectChilds(ITcSmTreeItem treeItem, ACClassInfoWithItems currentInfo)
        {
            foreach (ITcSmTreeItem item in ITcSmTreeItemList(treeItem))
            {
                ...
                string tItemName = ITCAction(() => item.Name).Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault();
        ...
        {
            _mainDecl = String.Empty;
            if (CurrentACProject.ACProjectType == Global.ACProjectTypes.Application)
            {
                ITcSmTreeItem main = FindTreeItem(GCL.cMainIdentifier, CurrentPOU);
                string decl = ITCAction(() => main is ITcPlcDeclaration ? ((ITcPlcDeclaration)main).DeclarationText : String.Empty);

                int startIndex = decl.IndexOf(C_RegionStartName);
                int endIndex = decl.IndexOf(C_VBRegionEnd);
                ...
                if (startIndex != -1 && endIndex != -1)
                {
                    ...
                    {
                        int endIndexVB = declVB.IndexOf(C_VBRegionEnd);
                        if (endIndexVB < 0)
                            _mainDecl = declVB.Substring(C_RegionStartName.Length);
                        else
                            _mainDecl = declVB.Substring(C_RegionStartName.Length, endIndexVB);
                    }
                }
            }
        ...
        private void SyncRecursive(ITcSmTreeItem parent, ACClassInfoWithItems info, string parentName)
        {
            foreach (ACClassInfoWithItems element in info.Items)
            {
                ...
                string pouName = parentName + GCL.Delimiter_DirSeperator + element.ACIdentifier;
                ...
                if (element.Items.Any())
                {
                    ...
                    if (CurrentACProject.ACProjectType == Global.ACProjectTypes.Application && element.ACIdentifier != _acClassInfoMovedName)
                    {
                        ...
                        _acClassInfoMovedName = String.Empty;
                    }
                }
                else
                {
        ...
        private void SyncRecursiveChecked(ACClassInfoWithItems info, string parentName, ITcSmTreeItem parent = null)
        {
            ...
            {
                string pouName = parentName + GCL.Delimiter_DirSeperator + item.ACIdentifier;
                if (item.IsChecked)
                {
                    CurrentProgressInfo.TotalProgress.ProgressCurrent++;
        ...
        private ITcSmTreeItem CheckOrMakeFolder(ITcSmTreeItem tcItem, ACClassInfoWithItems info, out bool newFolder)
        {
            ...
            {
                newFolder = true;
                return ITCSmTreeItemCreateChild(tcItem, info.ACIdentifier, ItemType.Folder, String.Empty, null);
            }
        }
        ...
        private ITcSmTreeItem CheckOrMakePOU(ITcSmTreeItem tcItem, ACClassInfoWithItems info, string pouName)
        {
            ...
            {
                ITcSmTreeItem pou = ITCSmTreeItemCreateChild(tcItem, pouName, ItemType.Pou, String.Empty, ResolveVInfo(info));
                if (pou == null)
                    return null;
        ...
        private object ResolveVInfo(ACClassInfoWithItems info)
        {
            ...
            string extendsName = String.Empty;
            string implementName = String.Empty;
            ...
            vInfo[0] = GCL.cST_Keyword_ST;
            if (bExtends && bImplements)
            {
                vInfo[1] = GCL.cST_Keyword_Extends;
                vInfo[2] = extendsName;
                vInfo[3] = GCL.cST_Keyword_Implements;
            ...
            else
            {
                ...
                else
                {
                    if (bImplements)
                    {
                        vInfo[1] = GCL.cST_Keyword_Implements;
        ...
        private string FindExtendsName(ACClass acClass)
        {
            ...
            while (!isFind)
            {
                ...
                {
                    isFind = true;
                    name = name.Insert(0, acClass.ACClass1_ParentACClass.ACIdentifier + GCL.Delimiter_DirSeperator);
                }
                else
                {
                    acClass = acClass.ACClass1_ParentACClass;
                    name = name.Insert(0, acClass.ACIdentifier + GCL.Delimiter_DirSeperator);
                }
            }
        ...
        public const string C_RootDerivationDecl = GCL.cRootACIdentifier + " : " + GCL.cRootClass + "(0);";
        ...
        {
            ITcSmTreeItem vb = FindTreeItem(GCL.cRootClass, CurrentPOU);
            if (vb == null)
            {
                ...
                vb = ITCSmTreeItemCreateChild(CurrentPOU, GCL.cRootClass, ItemType.Pou, String.Empty, vInfo);

                if (!_mainDecl.Contains(C_RootDerivationDecl))
                    _mainDecl = _mainDecl.Insert(0, C_RootDerivationDecl + System.Environment.NewLine);
            ...
            string mainImplementation = C_VBRegionStart + System.Environment.NewLine + C_ST_Code_IF_Autocomp_THEN + System.Environment.NewLine +
                        C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_childinit + GCL.cSTCode_Call + System.Environment.NewLine +
                        C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_postinit + "(" + GCL.cRootACIdentifier + ");" + System.Environment.NewLine + GCL.cST_Keyword_ENDIF + System.Environment.NewLine + System.Environment.NewLine +
                        GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_RunMain + GCL.cSTCode_Call + System.Environment.NewLine + C_VBRegionEnd;
            ...
            else if (!mainImpl.Contains(mainImplementation))
            {
                using (StringReader reader = new StringReader(mainImpl))
                {
                    ...
                    do
                    {
                        ...
                        if (line != null)
                        {
                            if (!mainImpl.Contains(C_VBRegionStart) && !mainImpl.Contains(C_VBRegionEnd))
                            {
                                sb.AppendLine(mainImplementation);
                                sb.AppendLine(line);
                            }
                            else if (line.Equals(C_VBRegionStart) && !mainImpl.Contains(C_ST_Code_IF_Autocomp_THEN))
                            {
                                sb.AppendLine(line);
                                sb.AppendLine(C_ST_Code_IF_Autocomp_THEN);
                            }
                            else if (line.Equals(C_ST_Code_IF_Autocomp_THEN) && !mainImpl.Contains(C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_childinit + GCL.cSTCode_Call))
                            {
                                sb.AppendLine(line);
                                sb.AppendLine(C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_childinit + GCL.cSTCode_Call);
                            }
                            else if (line.Equals(C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_childinit + GCL.cSTCode_Call) && !mainImpl.Contains(C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_postinit + GCL.cSTCode_Call))
                            {
                                sb.AppendLine(line);
                                sb.AppendLine(C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_postinit + GCL.cSTCode_Call);
                            }
                            else if (line.Equals(C_4spaces + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_FB_postinit + GCL.cSTCode_Call) && !mainImpl.Contains(GCL.cST_Keyword_ENDIF))
                            {
                                sb.AppendLine(line);
                                sb.AppendLine(GCL.cST_Keyword_ENDIF);
                            }
                            else if (line.Equals(C_4spaces + GCL.cST_Keyword_ENDIF) && !mainImpl.Contains(GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_RunMain + GCL.cSTCode_Call))
                            {
                                sb.AppendLine(line);
                                sb.AppendLine(System.Environment.NewLine + GCL.cRootACIdentifier + GCL.cADSPathSeparator + GCL.cMethodName_RunMain + GCL.cSTCode_Call);
                            }
                            else
                                sb.AppendLine(line);
                        }
        ...
        private void RemoveDeletedItemsFromMain(ITcSmTreeItem main)
        {
            foreach(var edgeTuple in _edgeDeleteCache)
            {
                ...
                if (edgeIndex != -1)
                {
                    ...
                    _mainEdgeDecl = _mainEdgeDecl.Replace(edge, String.Empty);
                }
            }
            ...
            {
                int startIndex = _mainDecl.IndexOf(GCL.cADSInstancePrefix + deletedPou + " :");
                if (startIndex == -1)
        ...
        private void AddVBDeclarationAndImplementation(ITcSmTreeItem vb, string childName)
        {
            ...
            bool initialDecl = !declaration.Contains(C_VBRegionStart) && !declaration.Contains(C_VBRegionEnd);
            ...
            using (StringReader reader = new StringReader(declaration))
            {
                ...
                do
                {
                    ...
                    if (line != null)
                    {
                        if (initialDecl && line.Equals(GCL.cST_Keyword_VAR_INPUT))
                        {
                            sb.AppendLine(line);
                            sb.AppendLine(VBRegionStart(C_4spaces));
                            sb.AppendLine(C_8spaces + GCL.cADSInstancePrefix + childName + " :" + _ReferenceTo + childName + ";");
                            sb.AppendLine(C_4spaces + C_VBRegionEnd);
                        }
                        else if (line.Equals(C_4spaces + C_VBRegionEnd) && !declaration.Contains(C_8spaces + GCL.cADSInstancePrefix + childName + " :" + _ReferenceTo + childName + ";"))
                        {
                            sb.AppendLine(C_8spaces + GCL.cADSInstancePrefix + childName + " :" + _ReferenceTo + childName + ";");
                            sb.AppendLine(line);
                        }
            ...
            ITcSmTreeItem fbChildInit = FindTreeItem(GCL.cMethodName_FB_childinit, vb);
            if (fbChildInit == null)
            {
                string[] vInfo = new string[4];
                vInfo[0] = GCL.cST_Keyword_ST;
                vInfo[1] = String.Empty;
                vInfo[2] = GCL.cST_Keyword_PUBLIC;
                vInfo[3] = String.Empty;
                fbChildInit = ITCSmTreeItemCreateChild(vb, GCL.cMethodName_FB_childinit, ItemType.Method, String.Empty, vInfo);
            }
        ...
        private void AddPouDeclarationAndImplementation(ITcSmTreeItem parentPou, ITcSmTreeItem childPou, ACClassInfoWithItems info)
        {
            ...
            if (ITCAction(() => parentPou.ItemType) == (int)ItemType.Folder)
            {
                foreach (ITcSmTreeItem item in ITcSmTreeItemList(parentPou))
                    if (ITCAction(() => item.Name).Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() == ITCAction(() => parentPou.Name) && ITCAction(() => item.ItemType) == (int)ItemType.Pou)
                    {
            ...
            if (ITCAction(() => childPou.ItemType) == (int)ItemType.Folder)
            {
                foreach (ITcSmTreeItem childItem in ITcSmTreeItemList(childPou))
                    if (ITCAction(() => childItem.Name).Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() == ITCAction(() => childPou.Name) && ITCAction(() => childItem.ItemType) == (int)ItemType.Pou)
                    {
            ...
            if (ITCAction(() => parentCorrectPou.Name) == ITCAction(() => childCorrectPou.Name))
            {
                ...
                foreach (ITcSmTreeItem item in ITcSmTreeItemList(parent))
                    if (ITCAction(() => item.Name).Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() == ITCAction(() => parent.Name) && ITCAction(() => item.ItemType) == (int)ItemType.Pou)
                    {
                        parentCorrectPou = item;
                        break;
                    }
            ...
            if (!declaration.Contains(C_VBRegionStart) && !declaration.Contains(C_VBRegionEnd))
        ...
        {
            string childName = childFullName.Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault();

            List<ITcSmTreeItem> listChilds = ITcSmTreeItemList(pou);
            ITcSmTreeItem postInit = listChilds.FirstOrDefault(c => ITCAction(() => c.Name) == GCL.cMethodName_FB_childinit && ITCAction(() => c.ItemType) == (int)ItemType.Method);
            ...
            if (postInit == null)
            {
                ...
                vInfo[2] = GCL.cST_Keyword_PUBLIC;
                vInfo[3] = String.Empty;
                postInit = ITCSmTreeItemCreateChild(pou, GCL.cMethodName_FB_childinit, ItemType.Method, String.Empty, vInfo);
                string decl = ITCAction(() => postInit is ITcPlcDeclaration ? ((ITcPlcDeclaration)postInit).DeclarationText : null);
                decl = decl + System.Environment.NewLine + GCL.cST_Keyword_VAR_INPUT + System.Environment.NewLine + GCL.cST_Keyword_END_VAR;
            ...
            else
            {
                using (StringReader reader = new StringReader(implementation))
                {
                    ...
                    do
                    {
                        ...
                        {
                            sb.AppendLine(line);
                            if (line == C_RegionStartName && !implementation.Contains(GCL.cADSInstancePrefix + childName + C_ST_Code_RefMainChild + childFullName + ";"))
                            {
                                sb.AppendLine(C_4spaces + GCL.cADSInstancePrefix + childName + C_ST_Code_RefMainChild + childFullName + ";");
                                sb.AppendLine(C_4spaces + GCL.cMethodName_AddChild + "(" + GCL.cADSInstancePrefix + childName + ");");
                            }
                        }
                    }
        ...
        private string CreatePathToRootForTargetName(string targetName)
        {
            ...
            string[] parts = targetName.Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries);

            foreach (string part in parts)
            {
                generatedName += "._" + part;
            }
            generatedName = generatedName.Insert(0, GCL.cMainIdentifier + GCL.cADSPathSeparator + GCL.cRootACIdentifier);
        ...
        private string AddEdgeDeclaration(ITcSmTreeItem sourceItem, string targetName, string targetNameToMain, ACClassPropertyRelation connection, out string targetProperty)
        {
            ...
            string edgeInstance = string.Format(PAEdge.ClassName + "(_{0}._{1}, _{2}._{3}, ConnectionTypesEnum.{4})", instanceName, sourceProp, targetName, targetProp, connectionType);
        ...
        {
            List<ITcSmTreeItem> listChilds = ITcSmTreeItemList(pou);
            ITcSmTreeItem postInit = listChilds.FirstOrDefault(c => ITCAction(() => c.Name) == GCL.cMethodName_FB_postinit && ITCAction(() => c.ItemType) == (int)ItemType.Method);
            ...
            if (postInit == null)
            {
                ...
                vInfo[2] = GCL.cST_Keyword_PUBLIC;
                vInfo[3] = String.Empty;
                postInit = ITCSmTreeItemCreateChild(pou, GCL.cMethodName_FB_postinit, ItemType.Method, String.Empty, vInfo);
                string decl = ITCAction(() => postInit is ITcPlcDeclaration ? ((ITcPlcDeclaration)postInit).DeclarationText : null);
                decl = decl + System.Environment.NewLine + GCL.cST_Keyword_VAR_INPUT + System.Environment.NewLine + C_4spaces + "root : IACVariobatch;" + System.Environment.NewLine + GCL.cST_Keyword_END_VAR;
                ITCPlcDeclarationTextSet(postInit, decl);
            }
            ...
            else
            {
                ...
                if (!implementation.Contains(C_4spaces + C_ST_Code_IF_NOT_InitState_THEN))
                {
                    int indexOfVBRegEnd = implementation.IndexOf(C_VBRegionEnd);
                ...
                using (StringReader reader = new StringReader(implementation))
                {
                    ...
                    do
                    {
                        ...
                        if (line != null)
                        {
                            if (line == C_4spaces + GCL.cST_Keyword_ENDIF && !implementation.Contains(edgeName) && !callPostInit)
                            {
                                sb.AppendLine(C_8spaces + edgeName + C_ST_Code_RefMain + edgeName + ";");
                                sb.AppendLine(C_8spaces + propName + "." + GCL.cMethodName_AddEdge + "(" + edgeName + ");");
                                callPostInit = true;
                            }
                            sb.AppendLine(line);
                        }
        ...
        {
            string edgeInfo1 = edgeInfo + " :" + _ReferenceTo + PAEdge.ClassName + ";";
            ...
            using (StringReader reader = new StringReader(declaration))
            {
                ...
                do
                {
                    ...
                    if (line != null)
                    {
                        if (initialDeclaration && line.Equals(GCL.cST_Keyword_VAR))
                        {
                            sb.AppendLine(line);
                            sb.AppendLine(VBRegionStart(C_4spaces));
        ...
        private void AddProperty(ITcSmTreeItem pou, IEnumerable<ACClassProperty> properties)
        {
            foreach (var property in properties.Where(c => c.IsRPCEnabled))
            {
                ...
                vInfo[2] = GCL.cST_Keyword_PUBLIC;
                ITcSmTreeItem prop = ITCSmTreeItemCreateChild(pou, property.ACIdentifier, ItemType.Property, String.Empty, (object)vInfo);
        ...
        private void AddPropertySet(ITcSmTreeItem prop)
        {
            dynamic vInfo = new string[3];
            vInfo[0] = GCL.cST_Keyword_ST;
            vInfo[1] = GCL.cST_Keyword_PUBLIC;
            vInfo[2] = @"<ST><![CDATA[(* ST PropSet *)]]></ST>";

            prop.CreateChild(String.Empty, (int)ItemType.PropSet, String.Empty, (object)vInfo);
        }
        ...
        private void AddPropertyGet(ITcSmTreeItem prop)
        {
            ...
            prop.CreateChild(String.Empty, (int)ItemType.PropGet, String.Empty, (object)vInfo);
        }
        ...
        private void AddDefaultMethods(ITcSmTreeItem pou)
        {
            ...
            vInfo[0] = GCL.cST_Keyword_ST;
            vInfo[1] = GCL.cADSType_Bool;
            vInfo[2] = GCL.cST_Keyword_PUBLIC;
            vInfo[3] = String.Empty;

            ITcSmTreeItem methodInit = ITCSmTreeItemCreateChild(pou, GCL.cMethodName_FB_init, ItemType.Method, String.Empty, (object)vInfo);

            string declText = @"METHOD " + GCL.cMethodName_FB_init + " : " + GCL.cADSType_Bool + System.Environment.NewLine +
                    GCL.cST_Keyword_VAR_INPUT + System.Environment.NewLine +
                    "   bInitRetains : " + GCL.cADSType_Bool + "; // if TRUE, the retain variables are initialized (warm start / cold start)" + System.Environment.NewLine +
                    "   bInCopyCode : " + GCL.cADSType_Bool + ";  // if TRUE, the instance afterwards gets moved into the copy code (online change)" + System.Environment.NewLine +
                    "   parent : IACComponent;" + System.Environment.NewLine +
                    GCL.cST_Keyword_END_VAR + System.Environment.NewLine +
                    GCL.cST_Keyword_VAR + System.Environment.NewLine +
                    GCL.cST_Keyword_END_VAR;
        ...
        private void AddMethods(ITcSmTreeItem pou, IEnumerable<ACClassMethod> methods)
        {
            foreach (var method in methods.Where(c => c.IsRPCEnabled))
            {
                ...
                ITCSmTreeItemCreateChild(pou, method.ACIdentifier, ItemType.Method, String.Empty, vInfo);
            }
        }
        ...
        private string TranslateType(Type inType)
        {
            if (inType == typeof(bool))
                return GCL.cADSType_Bool;
            else if (inType == typeof(string))
                return GCL.cADSType_String;
            else if (inType == typeof(DateTime))
                return GCL.cADSType_DateTime;
            else if (inType == typeof(byte))
                return GCL.cADSType_Byte;
            else if (inType == typeof(TimeSpan))
                return GCL.cADSType_TimeSpan;
            else if (inType == typeof(short))
                return GCL.cADSType_Short;
            else if (inType == typeof(int))
                return GCL.cADSType_Int;
            else if (inType == typeof(ushort))
                return GCL.cADSType_UShort;
            else if (inType == typeof(uint))
                return GCL.cADSType_UInt;
            else if (inType == typeof(double))
                return GCL.cADSType_Double;
            else if (inType == typeof(float))
                return GCL.cADSType_Float;
            return String.Empty;
        }
        ...
        private void DeleteFromImplementationAndDeclaration(string pouName, ITcSmTreeItem parentItem)
        {
            ...
            if (ITCAction(() => parentItem.ItemType) == (int)ItemType.Folder)
            {
                ...
                {
                    if (ITCAction(() => item.Name).Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() == ITCAction(() => parentItem.Name))
                    {
                        parentTreePou = item;
                        break;
                    }
            ...
            ITcSmTreeItem fbChildInit = parentTreePou.LookupChild(GCL.cMethodName_FB_childinit);
        ...
        {
            ITcSmTreeItem fbPostInit = FindTreeItem(GCL.cMethodName_FB_postinit, pou);

            if (fbPostInit == null)
                return;

            string postInitImplementation = ITCAction(() => fbPostInit is ITcPlcImplementation ? ((ITcPlcImplementation)fbPostInit).ImplementationText : String.Empty);
        ...
        private void DeleteEdgeFromPouDeclarationAndImplementation(ITcSmTreeItem treeItem, string edgeName)
        {
            ...
            ITcSmTreeItem fbInit = FindTreeItem(GCL.cMethodName_FB_postinit, treeItem);
        ...
        private void CheckAndMoveAfterDelete(ITcSmTreeItem folder, ACClassInfoWithItems info)
        {
            if (ITCAction(() => folder.ItemType) == (int)ItemType.Folder && folder.ChildCount == 1)
            {
                ...
                ITcSmTreeItem pou = parent.CreateChild(pouName, (int)ItemType.Pou, String.Empty, GCL.cST_Keyword_ST);
        ...
        string _mainDecl = String.Empty;
        string _mainEdgeDecl = String.Empty;
        ...
        private void CreateMainDeclarationAndImplementation(string fullName)
        {
            ...
            if (!fullName.Contains(GCL.Delimiter_DirSeperator))
            {
                parentName = GCL.cRootClass;
                shortName = fullName;
            }
            else
            {
                parentName = fullName.Substring(0, fullName.Length - (fullName.Length - fullName.LastIndexOf(GCL.Delimiter_DirSeperator)));
                shortName = fullName.Substring(fullName.LastIndexOf(GCL.Delimiter_DirSeperator) + 3);
            }

            string instance = string.Format("_{0} : {0}(_{1});", fullName, parentName);

            if (!_mainDecl.Contains(instance))
                _mainDecl += System.Environment.NewLine + C_4spaces + instance;
        }
        ...
        {
            string[] parts = pouName.Split(_DirSeperatorArr, StringSplitOptions.RemoveEmptyEntries);
        ...
        private string FindPouName(ACClassInfoWithItems info)
        {
            ...
            {
                info = info.ParentACObject as ACClassInfoWithItems;
                name = name.Insert(0, info.ACIdentifier + GCL.Delimiter_DirSeperator);
            }
            return name;
        }
        ...
        private void SetIsCheckedOnCurrentProject(ACClassInfoWithItems currentProject, ITcSmTreeItem currentTreeItem)
        {
            ...
            {
                if (ITCAction(() => treeItem.Name).Contains(GCL.Delimiter_DirSeperator))
        ...
        private void SetCheckOnInfo(string tcPath, ACClassInfoWithItems currentProject)
        {
            ...
            delimiter[0] = GCL.Delimiter_DirSeperator;
        ...
        private void CopyPou(ITcSmTreeItem copyFrom, ITcSmTreeItem copyTo)
        {
            ...
            foreach (ITcSmTreeItem item in ITcSmTreeItemList(copyFrom))
            {
                ...
                if (copyToItem == null)
                    copyToItem = ITCSmTreeItemCreateChild(copyTo, ITCAction(() => item.Name), (ItemType)ITCAction(() => item.ItemType), String.Empty, GCL.cST_Keyword_ST);
                //copyToItem = copyTo.CreateChild(ITCSmTreeItemName(item), item.ItemType, String.Empty, GCL.cST_Keyword_ST);
                ...
                if (ITCAction(() => item.ItemType) == (int)ItemType.Property)
                {
                    ...
                    foreach (ITcSmTreeItem itemProp in ITcSmTreeItemList(item))
                    {
                        ...
                        if (copyToItemProp == null)
                            copyToItemProp = ITCSmTreeItemCreateChild(copyToItem, ITCAction(() => itemProp.Name), (ItemType)ITCAction(() => itemProp.ItemType), String.Empty, GCL.cST_Keyword_ST);
