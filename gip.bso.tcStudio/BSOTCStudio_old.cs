...
namespace gip.bso.tcStudio
{
    ...
    public class BSOTCStudio_old : ACBSO
    {
        #region c'tors

        public BSOTCStudio_old(ACClass acType, IACObject content, IACObject parentACObject, ACValueList parameter, string acIdentifier) : base(acType, content, parentACObject, parameter, acIdentifier)
        {
        ...
        {
            _ACProjectManager = new ACProjectManager(Database.ContextIPlus, Variobatch);
        ...
        public ACProject CurrentACProject
        {
            ...
            set
            {
                ...
                if (value != null)
                {
                    ...
                    _CurrentACProject = _ACProjectManager.LoadACProject(value.ACProjectID, new Type[4]{typeof(PAProcessFunction), typeof(PAProcessModule), typeof(ApplicationManager), typeof(PAModule)});
                    _CurrentProjectItemRoot = _ACProjectManager.CurrentProjectItemRoot;
                    _CountProjectItems = 0;
                    CountRecursive(CurrentProjectItemRoot);
                    if (CurrentPOU != null)
                    {
        ...
        private ACClassInfoWithItems _CurrentProjectItemRoot;
        ...
        public ACClassInfoWithItems CurrentProjectItemRoot
        {
            ...
            set
            {
                ...
                OnPropertyChanged("ProjectItemRoot");
            }
        }
                ...
                if (pou != null)
                {
                    foreach (ITcSmTreeItem tcMethod in ITcSmTreeItemList(pou))
                    {
                        ...
                        if (ITCAction(() => tcMethod.ItemType) == (int)ItemType.Method && itemName != "FB_init" && itemName != "FB_reinit" && itemName != "FB_exit"
                                                                      && itemName != "FB_postinit" && itemName != "OnMainEnter" && itemName != "OnMainExit"
        ...
        private void CompareTCProject(ITcSmTreeItem treeItem, ACClassInfoWithItems currentInfo)
        {
            ...
            if (ITCSmTreeItemChildCount(treeItem) - 1 > currentInfo.Items.OfType<ACClassInfoWithItems>().Count(c => (Global.ChangeTypeEnum)c.IconState != Global.ChangeTypeEnum.Added))
            {
                foreach (ITcSmTreeItem tItem in ITcSmTreeItemList(treeItem))
                {
                    ...
                    {
                        ACClassInfoWithItems newInfo = new ACClassInfoWithItems() { ACCaption = tItemName, IconState = Global.ChangeTypeEnum.Removed, IsEnabled = true, 
        ...
        public void SyncProjects()
        {
            ...
            else
            {
                ...
                ProjectItemFilter filter = new ProjectItemFilter() { SearchClassText = "" };
                CurrentACProject = _ACProjectManager.LoadACProject(CurrentACProject.ACProjectID, true, true, true, true, null, true, false, filter, ACProjectItemPreparedSearchFunctions.SearchACClassText, ACProjectManager.ProjectTreeType.Project);
        ...
        private void SyncProj()
        {
            ...
            {
                CurrentProgressInfo.TotalProgress.ProgressRangeTo = CurrentProjectItemRoot.GetCountRecursive();
            ...
            if (CurrentACProject.ACProjectType == Global.ACProjectTypes.Applicationproject)
            {
        ...
        {
            foreach (var element in info.Items.OfType<ACClassInfoWithItems>())
            {
                ...
                {
                    DeleteEdgeFromSource(FindTreeItem(pouName, parent));
                ...
                if (element.Items.Any())
                {
                    ...
                    if (CurrentACProject.ACProjectType == Global.ACProjectTypes.Applicationproject && element.ACIdentifier != _acClassInfoMovedName)
                    {
                ...
                else
                {
                    ...
                    if (childItem != null && CurrentACProject.ACProjectType == Global.ACProjectTypes.Applicationproject)
        ...
        private void AddMainDeclarationAndImplementation(ITcSmTreeItem main, string childName)
        {
            ...
            if(vb == null)
            {
                ...
                //string[] vInfoFB_init = new string[4];
                //vInfoFB_init[0] = "ST";
                //vInfoFB_init[1] = "BOOL";
                //vInfoFB_init[2] = "PUBLIC";
                //vInfoFB_init[3] = "";
                //ITCSmTreeItemCreateChild(vb, "FB_init", ItemType.Method, "", vInfoFB_init);


                string mainDeclaration = ITCAction(() => main is ITcPlcDeclaration ? ((ITcPlcDeclaration)main).DeclarationText : null);

                using (StringReader reader = new StringReader(mainDeclaration))
                {
                    ...
                    do
                    {
                        ...
                        if (line != null)
                        {
                            if (line.Equals("VAR") && !mainDeclaration.Contains(_VBRegionStart) && !mainDeclaration.Contains(_VBRegionEnd))
                            {
                                sb.AppendLine(line);
                                sb.AppendLine(VBRegionStart(_4spaces));
                                sb.AppendLine(_8spaces + "_VB : VB(0);");
                                sb.AppendLine(_4spaces + _VBRegionEnd);
                            }
                            else if (line.Equals(_VBRegionEnd) && !mainDeclaration.Contains(_8spaces + "_VB : VB(0);"))
                            {
                                sb.AppendLine(_8spaces + "_VB : VB(0);");
                                sb.AppendLine(line);
                            }
                    ...
                    while (line != null);
                    ITCPlcDeclarationTextSet(main, sb.ToString());
                }
                    ...
                    using (StringReader reader = new StringReader(mainImpl))
                    {
                        ...
                        do
                        {
                            ...
                            if (line != null)
                            {
                                ...
                                else if (line.Equals("IF Automation_components_gip.GVL.gInstanceIDCount = 0 THEN") && !mainImpl.Contains(_4spaces + "_VB.FB_postinit();"))
                                {
                                    sb.AppendLine(line);
        ...
        private void AddVBDeclarationAndImplementation(ITcSmTreeItem vb, string childName)
        {
            ...
            using (StringReader reader = new StringReader(declaration))
            {
                ...
                do
                {
                    ...
                    if (line != null)
                    {
                        if (initialDecl && line.Equals("VAR_OUTPUT"))
                        {
                            sb.AppendLine(line);
                            sb.AppendLine(VBRegionStart(_4spaces));
                            sb.AppendLine(_8spaces + "_" + childName + " : " + childName + "(THIS^);");
                            sb.AppendLine(_4spaces + _VBRegionEnd);
                        }
                        else if (line.Equals(_4spaces + _VBRegionEnd) && !declaration.Contains(_8spaces + "_" + childName + " : " + childName + "(THIS^);"))
                        {
                            sb.AppendLine(_8spaces + "_" + childName + " : " + childName + "(THIS^);");
                            sb.AppendLine(line);
                        }
            ...
            ITcSmTreeItem fbInit = FindTreeItem("FB_init", vb);
            ...
            {
                implementation = _VBRegionStart + System.Environment.NewLine + _4spaces + "AddChild(_" + childName + ");" + System.Environment.NewLine + _VBRegionEnd;
                ITCPlcImplementationTextSet(fbInit, implementation);
            }
            else
            {
                using (StringReader reader = new StringReader(implementation))
                {
                    ...
                    do
                    {
                        ...
                        if (line != null)
                        {
                            ...
                            else if (line.Equals(_VBRegionEnd) && !implementation.Contains(_4spaces + "AddChild(_" + childName + ");"))
                            {
                                sb.AppendLine(_4spaces + "AddChild(_" + childName + ");");
                                sb.AppendLine(line);
                            }
        ...
        private string CreateAddChildDeclaration(ITcSmTreeItem parentPou, ITcSmTreeItem childPou, bool initialDeclaration = false)
        {
            ...
            using (StringReader reader = new StringReader(declaration))
            {
                ...
                do
                {
                    ...
                    {
                        sb.AppendLine(line);
                        if(initialDeclaration && line.Equals("VAR_OUTPUT"))
                        {
                            sb.AppendLine(VBRegionStart(_4spaces));
                            string childName = _8spaces + "_" + ITCAction(() => childPou.Name).Split(delimiter, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() + " : " + ITCAction(() => childPou.Name) + "(THIS^);";
                            sb.AppendLine(childName);
                            AddChildFBInitImplementation("_"+ ITCAction(() => childPou.Name).Split(delimiter, StringSplitOptions.RemoveEmptyEntries).LastOrDefault(), parentPou);
                            sb.AppendLine(_4spaces + _VBRegionEnd);
                        }
        ...
        /// <param name="pou">The POU.</param>
        private void AddChildFBInitImplementation(string childName, ITcSmTreeItem pou)
        {
            foreach(ITcSmTreeItem item in ITcSmTreeItemList(pou))
            {
                if(ITCAction(() => item.Name) == "FB_init" && ITCAction(() => item.ItemType) == (int)ItemType.Method)
                {
                    string implementation = ITCAction(() => item is ITcPlcImplementation ? ((ITcPlcImplementation)item).ImplementationText : null);
                    string createdImplementation;
                    if (string.IsNullOrEmpty(implementation))
                    {
                        createdImplementation = _VBRegionStart + System.Environment.NewLine + _4spaces + "AddChild(" + childName + ");" + System.Environment.NewLine + _VBRegionEnd;
                    }
                    else
                    {
                        using (StringReader reader = new StringReader(implementation))
                        {
                            ...
                            do
                            {
                                ...
                                {
                                    sb.AppendLine(line);
                                    if (line == "//VBRegionStart" && !implementation.Contains(childName))
                                        sb.AppendLine(_4spaces + "AddChild("+childName+");");
                                }
                            }
                            while (line != null);
                    ...
                    ITCPlcImplementationTextSet(item, createdImplementation);
                    break;
                }
            }
        }
            ...
            foreach (ACClassInfoWithItems item in info.Items)
            {
                                     ...
                                     && item.ValueT.ACClassPropertyRelation_SourceACClass.Any(c => c.ConnectionTypeIndex == (short)Global.ConnectionTypes.ConnectionPhysical)))
                {
                                                                                                           ...
                                                                                                           || c.ConnectionTypeIndex == (short)Global.ConnectionTypes.LogicalBridge))
                    {
                        ...
                        {
                            string edge = AddEdgeDeclaration(sourceItem, targetNameDecl, connection);
                            if (!string.IsNullOrEmpty(edge))
                            {
                                edge = edge.Insert(0, "VBEdgeInfo:");
                                edge = edge + "@" + sourcePouName;
                                AddEdgeInfoOnTarget(target, edge);
                            }
                        }
                        else
                        {
        ...
        private string AddEdgeDeclaration(ITcSmTreeItem sourceItem, string targetName, ACClassPropertyRelation connection)
        {
                ...
                do
                {
                    ...
                    if (line != null)
                    {
                        if (initialDeclaration && line.Equals("VAR"))
                        {
                            sb.AppendLine(line);
                            sb.AppendLine(VBRegionStart(_4spaces));
                            string edge = _8spaces + "_Edge001 : PAEdge(" + "_" + sourceProp + ", " + targetName + "." + "_" + targetProp + ", ConnectionTypesEnum." + connectionType + ");";
                            sb.AppendLine(edge);
                            sb.AppendLine(_4spaces + _VBRegionEnd);
                            AddEdgeFBPostInitImplementation("_Edge001", targetName, sourceItem, sourceProp, targetProp);
                            currentEdge = "_Edge001";
                        }
                        else if (line.StartsWith(_8spaces + "_Edge"))
                        {
                            sb.AppendLine(line);
                            currentEdgeNum = line.Trim().Split(':').FirstOrDefault();
                            currentEdgeNum = currentEdgeNum.Substring(currentEdgeNum.Length - 3);
                        }
                        else if (line == _4spaces + _VBRegionEnd)
                        {
                            ...
                            if (!declaration.Contains(partEdge))
                            {
                                int tempNum = 0;
                                if (string.IsNullOrEmpty(currentEdgeNum))
                                    currentEdgeNum = "001";
                                else if(int.TryParse(currentEdgeNum, out tempNum))
                                {
                                    tempNum++;
                                    if (tempNum.ToString().Length == 1)
                                        currentEdgeNum = "00" + tempNum;
                                    else if (tempNum.ToString().Length == 2)
                                        currentEdgeNum = "0"+tempNum;
                                    else
                                        currentEdgeNum = tempNum.ToString();
                                }
                                string edge = _8spaces + "_Edge" + currentEdgeNum + " : " + partEdge;
                                currentEdge = "_Edge" + currentEdgeNum;
                                sb.AppendLine(edge);
                                AddEdgeFBPostInitImplementation("_Edge" + currentEdgeNum, targetName, sourceItem, sourceProp, targetProp);
                            }
                            sb.AppendLine(line);
                        }
                        else
        ...
        private void AddEdgeFBPostInitImplementation(string edgeName, string targetName, ITcSmTreeItem pou, string sourcePropName, string targetPropName)
        {
            ...
            if (string.IsNullOrEmpty(implementation))
            {
                createdImplementation = _VBRegionStart + System.Environment.NewLine +
                                        ...
                                        + System.Environment.NewLine +
                                        _8spaces + sourcePropName + ".AddEdge(" + edgeName + ");"
                                        + System.Environment.NewLine +
                                        _8spaces + targetName +"."+ targetPropName+ ".AddEdge(" + edgeName + ");"
                                        + System.Environment.NewLine +
                                        _4spaces + "END_IF"
                ...
                using (StringReader reader = new StringReader(implementation))
                {
                    ...
                    do
                    {
                        ...
                        if (line != null)
                        {
                            ...
                            {
                                sb.AppendLine(_8spaces + sourcePropName+ ".AddEdge(" + edgeName + ");" + System.Environment.NewLine +
                                              _8spaces + targetName + "."+targetPropName+".AddEdge(" + edgeName + ");");
                                callPostInit = true;
                            }
                            sb.AppendLine(line);
        ...
        private void AddEdgeInfoOnTarget(ITcSmTreeItem target, string edgeInfo)
        {
                ...
                do
                {
                    ...
                    if (line != null)
                    {
                        if (initialDeclaration && line.Equals("VAR"))
                        {
                            sb.AppendLine(line);
                            sb.AppendLine(VBRegionStart(_4spaces));
                            sb.AppendLine(_8spaces + "//" + edgeInfo);
                            sb.AppendLine(_4spaces + _VBRegionEnd);
                        }
                        else if (line == _4spaces + _VBRegionEnd)
                        {
        ...
        private void DeleteFromImplementationAndDeclaration(string pouName, ITcSmTreeItem parentItem)
        {
            ...
            ITcSmTreeItem fbInit = parentTreePou.LookupChild("FB_init");
        ...
        /// </summary>
        /// <param name="target">The target POU.</param>
        private void DeleteEdgeFromSource(ITcSmTreeItem target)
        {
            if (target == null)
                return;
            ...
            if (ITCAction(() => target.ItemType) == (int)ItemType.Folder)
            {
                ...
                //}
                //return;
                target = ITcSmTreeItemList(target).FirstOrDefault(c => c.Name.Split(delimiter, StringSplitOptions.RemoveEmptyEntries).LastOrDefault() == ITCAction(() => target.Name));
                if (target == null)
                    return;
            }    

            string declaration = ITCAction(() => target is ITcPlcDeclaration ? ((ITcPlcDeclaration)target).DeclarationText : null);
            if(declaration.Contains("//VBEdgeInfo:"))
            {
                using(StringReader reader = new StringReader(declaration))
                {
                    string line = "";
                    do
                    {
                        ...
                        if(line != null)
                        {
                            if (line.Contains("//VBEdgeInfo:"))
                            {
                                string edge = line.Split(':').Last();
                                string edgeNo = edge.Split('@').FirstOrDefault();
                                string sourcePath = edge.Split('@').LastOrDefault();
                                ITcSmTreeItem source = FindPouFromRoot(sourcePath);
                                if(source != null)
                                    DeleteEdgeFromSourceDeclarationAndImplementation(source, edgeNo);
                            }
                        }
                    }
                    while (line != null);
        ...
        /// <summary>
        /// Delete edge declaration and implementation from source.
        /// </summary>
        /// <param name="treeItem">The source POU.</param>
        /// <param name="edgeName">The edge name.</param>
        private void DeleteEdgeFromSourceDeclarationAndImplementation(ITcSmTreeItem treeItem, string edgeName)
        {
            ...
            ITcSmTreeItem fbInit = FindTreeItem("FB_init", treeItem);
            string implementation = ITCAction(() => fbInit is ITcPlcImplementation ? ((ITcPlcImplementation)fbInit).ImplementationText : null);
            using (StringReader reader = new StringReader(implementation))
            {
                string line = "";
        ...
        private void CountRecursive(ACClassInfoWithItems info)
        {
            _CountProjectItems++;
            foreach(ACClassInfoWithItems item in info.Items)
            {
