    ...
    public static class GS1
    {
        ...
        public struct ParseResult
        {
            ...
            public double? DoubleResult
            {
                get
                {
                    if (DecimalPlaces.HasValue && StringResult != null)
                    {
                        string forParse = StringResult.Insert(StringResult.Length - DecimalPlaces.Value, CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator);
                        double result;
                        if (double.TryParse(forParse, out result))
                        {
                            return result;
                        }
                    }
        ...
        public static AII GetAII(string AI)
        {
            AII result = null;

            aiiDict.TryGetValue(AI, out result);
        ...
        public static Dictionary<AII, ParseResult> Parse(string data, bool hasCheckSum = true, bool throwException = false)
        {
            ...
            Dictionary<AII, ParseResult> result = new Dictionary<AII, ParseResult>();
            int index = 0;
            ...
            while (index < data.Length)
            {
                ...
                int? decimalPos = null;
                var ai = GetAI(data, ref index, out decimalPos);
                if (ai == null)
                {
                    if (throwException)
        ...
        private static AII GetAI(string data, ref int index, out int? decimalPlaces, bool usePlaceHolder = false)
        {
            ...
            for (int i = minLengthOfAI; i <= maxLengthOfAI; i++)
            {
                ...
                if (usePlaceHolder)
                {
                    string last = ai.LastOrDefault().ToString();
                    int places = 0;
                    if (int.TryParse(last, out places))
                    {
            ...
            if (!usePlaceHolder)
                result = GetAI(data, ref index, out decimalPlaces, true);
            return result;
        }
