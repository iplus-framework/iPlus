...
namespace HTMLConverter
{
    ...
    public static class HtmlFromXamlConverter
    {
        ...
        {
            StringBuilder htmlStringBuilder  = new StringBuilder(100);
            using (XmlTextReader xamlReader = new XmlTextReader(new StringReader(xamlString)))
            using (StringWriter sw1 = new StringWriter(htmlStringBuilder))
            using (XmlTextWriter htmlWriter = new XmlTextWriter(sw1))
            {
