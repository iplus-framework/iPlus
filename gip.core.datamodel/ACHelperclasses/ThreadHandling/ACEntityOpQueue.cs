...
namespace gip.core.datamodel
{
    ...
    public class ACEntityOpQueue<T> : ACDelegateQueue, IACEntityOpQueue where T : IACEntityObjectContext
    {
        ...
        protected void SetIsolationLevelUncommitedRead()
        {
            using (ACMonitor.Lock(Context.QueryLock_1X000))
            {
                Context.ExecuteStoreCommand(ACObjectContextHelper.SET_READ_UNCOMMITED);
            }
            if (Context.IsSeparateIPlusContext)
            {
                using (ACMonitor.Lock(Context.ContextIPlus.QueryLock_1X000))
                {
                    Context.ContextIPlus.ExecuteStoreCommand(ACObjectContextHelper.SET_READ_UNCOMMITED);
                }
            }
            _UncommitedRead = 1;
        }
        ...
        protected void SetIsolationLevelCommitedRead()
        {
            using (ACMonitor.Lock(Context.QueryLock_1X000))
            {
                Context.ExecuteStoreCommand(ACObjectContextHelper.SET_READ_COMMITED);
            }
            if (Context.IsSeparateIPlusContext)
            {
                using (ACMonitor.Lock(Context.ContextIPlus.QueryLock_1X000))
                {
                    Context.ContextIPlus.ExecuteStoreCommand(ACObjectContextHelper.SET_READ_COMMITED);
                }
            }
            _UncommitedRead = 0;
        }
            ...
            try
            {
                ...
                using (ACMonitor.Lock(Context.QueryLock_1X000))
                {
                    try
                    {
                        ...
                        {
                            MsgWithDetails msg = Context.ACSaveChanges(true, _TransScope == null ? SaveOptions.AcceptAllChangesAfterSave : SaveOptions.None, SaveChangesWithoutValidation);
                            if (msg != null)
                            {
                ...
                if (Context.IsSeparateIPlusContext)
                {
                    ...
                    using (ACMonitor.Lock(Context.ContextIPlus.QueryLock_1X000))
                    {
                        try
                        {
                            ...
                            {
                                MsgWithDetails msg = Context.ContextIPlus.ACSaveChanges(true, _TransScope == null ? SaveOptions.AcceptAllChangesAfterSave : SaveOptions.None, SaveChangesWithoutValidation);
                                if (msg != null)
                                {
