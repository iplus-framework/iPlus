...
namespace gip.core.autocomponent
{
    ...
    [ACClassInfo(Const.PackName_VarioSystem, "en{'PAClassAlarmingBase'}de{'PAClassAlarmingBase'}", Global.ACKinds.TACAbstractClass, Global.ACStorableTypes.Required, false, true, 
        Description = "Abstract base class that provides comprehensive alarm management functionality for ACComponents in industrial automation systems. This class implements a complete alarm lifecycle management system including alarm generation, tracking, acknowledgment, persistence, and hierarchical propagation. Components inheriting from this class can raise alarms using OnNewAlarmOccurred() when fault conditions occur and clear them with OnAlarmDisappeared() when conditions return to normal. The class maintains local alarm collections, supports alarm filtering, provides event-driven notifications through AlarmChangedEvent and SubAlarmsChangedEvent, automatically persists alarms to the MsgAlarmLog database table, and propagates alarm states up the component hierarchy so parent components are aware of child alarms. It includes built-in interaction methods for alarm acknowledgment (AcknowledgeAlarms, AcknowledgeAllAlarms, AcknowledgeSubAlarms), alarm querying (GetAlarms, GetAlarmsConfig), and state backup/restore functionality. The class also provides network properties HasAlarms and AlarmsAsText for real-time alarm status monitoring and supports alarm transaction grouping for batch alarm processing. Use this as the base class for any component that needs to generate, manage, and respond to alarm conditions in your automation system, ensuring consistent alarm handling behavior across all system components.")]
