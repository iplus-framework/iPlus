...
namespace gip.core.processapplication
{
    ...
    public class PAEActuator2way_In : PAEActuatorBase
    {
        #region c'tors

        static PAEActuator2way_In()
        {
            RegisterExecuteHandler(typeof(PAEActuator2way_In), HandleExecuteACMethod_PAEActuator2way_In);
        }

        public PAEActuator2way_In(ACClass acType, IACObject content, IACObject parentACObject, ACValueList parameter, string acIdentifier="")
        ...
        public override void ActivateRouteItemOnSimulation(RouteItem item, bool switchOff)
        {
            ...
            if (PAPointMatIn1.ConnectionList.Any(c => c.SourceParentComponent == item.SourceACComponent))
            {
                if (!switchOff)
                    OnSetPosition1Values();
            }
            else if (PAPointMatIn2.ConnectionList.Any(c => c.SourceParentComponent == item.SourceACComponent))
            {
                if (!switchOff)
                    OnSetPosition2Values();
            }
        ...
        public static bool HandleExecuteACMethod_PAEActuator2way_In(out object result, IACComponent acComponent, string acMethodName, ACClassMethod acClassMethod, object[] acParameter)
        {
